...
#
# Added support for multi-column output from MultiCell method (Tom McGehee)
# To use, call StartColumns(number_of_columns,gutter_width) to begin multi-column output;
#         call StopColumns to end multi-column output
# Column width is automatically computed, and overrides the 'w' parameter in MultiCell.
#
...
class FPDF
    ...
    def SetCol(column)
      @col = column
      w=@w-@rMargin-@lMarginSave
      g = (@cols-1) * @gutter
      @cw = (w-g)/@cols
      x=@lMarginSave + (@cw*(column-1)) + (@gutter*(column-1))
      self.SetLeftMargin(x)
      self.SetXY(x,@col_y)     
    end
    
    def StartColumns(cols=1,gutter=5)
      @cols = cols
      @gutter = gutter
      @col = 1      
      @col_y = self.GetY
    ...
    def AcceptPageBreak
        ...
        if @AutoPageBreak && @cols > 1 && @col_active
          if @col < @cols
              self.SetCol(@col + 1)
              return false
          else
    ...
    def Cell(w,h=0,txt='',border=0,ln=0,align='',fill=0,link='')
        ...
        if @y+h>@PageBreakTrigger and !@InFooter and self.AcceptPageBreak
            ...
            @col_y = self.GetY if @col_active
            @inpageoverflow = false
            @x=x
            if ws>0
                @ws=ws
    ...
    def MultiCell(w,h,txt,border=0,align='J',fill=0)
        ...
        cw=@CurrentFont['cw']
        w=@w-@rMargin-@x if w==0
        #override w with computed width if multi-column output unless in footer
        w=@cw if @col_active and !@InFooter
