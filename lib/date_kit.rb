...
class Date
  ...
  def self.parse_date(string)
    ...
    elsif string =~ /^(\d{2})[-\/\.](\d{2})[-\/\.](\d{4}|\d{2})$/      
      # mm dd yyyy formats    
      x = Date.new($3.to_i,$1.to_i,$2.to_i)
      begin
        result = Date.parse(x.to_s,true)
      rescue
        raise ArgumentError
      end      
  ...
  def validates_dates(*attr_names)
    configuration =
      { :message => 'is an invalid date ' \
                    '(examples of valid date formats: 23, 23 feb, 23 feb 07, ' \
                    '6 feb 2007, 2007-02-23, 02/12/07, 02/12/2007 (separators may be "/" or "." or "-"), '\
                    ' +6days, +6d, +6, +2w, -6m, +1y)',
        :on => :save,
      }
