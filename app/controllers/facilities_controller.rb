class FacilitiesController < ApplicationController
  ...
  def list      
    ...
    oldparams = {}
    #ideally, we'd have a list of the keys for this and make it a one liner
    #still losing hidden columns, first three work.
    if params[:clear] == "filter"
      oldparams[:state] = session[:filter][:state] if session[:filter][:state]
      oldparams[:linesperpage] = session[:filter][:linesperpage] if session[:filter][:linesperpage]
      oldparams[:fac_sort] = session[:filter][:fac_sort] unless session[:filter][:fac_sort].nil?
      oldparams[:fac_column] = session[:filter][:fac_columns] unless session[:filter][:fac_columns].nil?
      session[:filter] = nil
    end
    session[:filter] = nil if params[:clear] == "true"
    filter_set_defaults
    oldparams.each{|k,v|session[:filter][k] = v}
...
class CATPDF < FPDF
  def Header
    ...
    title = "Resource Guide of Social Services"
  ...
  # Page number
  MultiCell(0,4, "Brought to you with the help of: FEMA, the American Red Cross, 211 and the 
United Way, the Hancock County Food Pantry, Hope Haven Children's Homes, 
Lagniappe Presbyterian Church, Mississippi Center for Nonprofits, the 
Katrina Volunteers Hancock County Yahoo Group,  the University of 
Pennsylvania's School of Social Policy & Practice, and the Citizen Action Team's Relief Database" , 'T', 'C')  
